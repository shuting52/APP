  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        {/* Cultural Subtitle: Chinese Lunar Date & Solar Term Banner */}
        <LunarCulturalBar />
      ...
      <BottomFloatingNav
        onGoHome={() => {
          ...
          setSearchQuery('');
          showToast('已返回陈淑婷精选首页');
        }}
        onOpenCategories={() => setIsCategoryPickerOpen(true)}
        onOpenAbout={() => setIsAboutOpen(true)}
        onShare={() => {
          ...
          if (navigator.share) {
            navigator.share({
              ...
              url: shareUrl
            }).then(() => showToast('感谢分享！')).catch(() => {
              navigator.clipboard.writeText(shareUrl);
              showToast('已复制主页链接，快去分享给好友吧！');
            });
          } else {
            navigator.clipboard.writeText(shareUrl);
            showToast('已复制主页链接，快去分享给好友吧！');
